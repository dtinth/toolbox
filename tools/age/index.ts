...
export default function init(api: Api) {
  ...
  const choosePasskeySource = async (): Promise<PasskeySource | null> => {
    if (hasIdentity() || !webauthnSupported()) {
      return "new";
    }
  ...
  const persist = async (secret: string, how: string, source: PasskeySource) => {
    try {
      ...
      toast(`Could not save identity: ${(error as Error).message}`);
    }
  };
  ...
  const copyPublic = async () => {
    ...
    if (r === null || r === "") {
      toast("No identity yet — generate or import one");
      return;
    }
  ...
  const copySecret = async () => {
    ...
    try {
      ...
      toast(`Could not unlock: ${(error as Error).message}`);
    }
  };
  ...
  const doEncrypt = async () => {
    if (!input) {
      toast("Choose a file to encrypt");
      return;
    }
    ...
    const recipients = addSelf && own !== null && own !== "" ? [...new Set([...base, own])] : base;
    if (recipients.length === 0) {
      toast("Add a recipient (age1…) or generate an identity first");
      return;
    }
    const f = input;
    try {
      ...
      toast(`Encryption failed: ${(error as Error).message}`);
    }
  };

  const doDecrypt = async () => {
    if (!input) {
      toast("Choose an .age file to decrypt");
      return;
    }
    ...
    if (typed !== "") {
      ...
      toast("Paste an AGE-SECRET-KEY-1… or set up an identity first");
      return;
    }
    const f = input;
    try {
      ...
      toast(`Decryption failed — wrong key or not an age file (${(error as Error).message})`);
    }
  };
  ...
  const doReencrypt = async () => {
    if (!input) {
      toast("Choose an .age file to re-encrypt");
      return;
    }
    const own = storedRecipient();
    if (own === null || own === "") {
      toast("Set up an identity first — re-encryption needs your key");
      return;
    }
    const typed = parseRecipients(recipientText);
    ...
    try {
      ...
      toast(`Re-encryption failed — wrong key or not an age file (${(error as Error).message})`);
    }
  };
  ...
  api.onRender = () => {
    ...
    if (mode === "encrypt") {
      fileLabel = "Choose, drop, or paste a file to encrypt";
    } else if (mode === "decrypt") {
      fileLabel = "Choose, drop, or paste an .age file to decrypt";
    } else {
      fileLabel = "Choose, drop, or paste an .age file to re-encrypt";
    }
    api.ui.file(input, {
      label: fileLabel,
      onFile: (f) => {
    ...
    if (mode === "encrypt") {
      api.ui.textInput(recipientText, {
        placeholder:
          own !== null && own !== "" ? "Recipient age1… (blank = your own key)" : "Recipient age1…",
        onChange: (v) => {
          recipientText = v;
        },
      });
    ...
    let outputPlaceholder: string;
    if (mode === "encrypt") {
      outputLabel = "Encrypted output:";
      outputPlaceholder = "Encrypted file appears here";
    } else if (mode === "decrypt") {
      outputLabel = "Decrypted output:";
      outputPlaceholder = "Decrypted file appears here";
    } else {
      outputLabel = "Re-encrypted output:";
      outputPlaceholder = "Re-encrypted file appears here";
    }
